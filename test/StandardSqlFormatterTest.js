...
describe("StandardSqlFormatter", function () {
    ...
    it("recognizes [] strings", function () {
        ...
        expect(sqlFormatter.format("[foo ]] JOIN bar]")).toBe(
            "[foo ]] JOIN bar]",
        );
    });
    ...
    it("handles materialized view", () => {
        const materializedViewQuery = `
CREATE MATERIALIZED VIEW test.schema.materializedview SCHEDULE CRON '0 0 10 ? * *' AT TIME ZONE 'America/Los_Angeles' COMMENT 'This is my table comment' TBLPROPERTIES('test.key2' = 'test.value2') AS SELECT tpep_pickup_datetime AS pickup_time, AVG(trip_distance) AS avg_trip_distance FROM samples.nyctaxi.trips GROUP BY 1 ORDER BY 1
`;
        const expected = `
CREATE MATERIALIZED VIEW
  test.schema.materializedview
  SCHEDULE CRON '0 0 10 ? * *' AT TIME ZONE 'America/Los_Angeles'
  COMMENT 'This is my table comment'
  TBLPROPERTIES('test.key2' = 'test.value2') AS
SELECT
...
ORDER BY
  1`;
        expect(sqlFormatter.format(materializedViewQuery.trim())).toBe(
            expected.trim(),
        );
    });

    it("handles streaming table with OR REFRESH", () => {
        const streamingTableQuery = `CREATE OR REFRESH STREAMIMG TABLE test.schema.materializedview SCHEDULE CRON '0 0 10 ? * *' AT TIME ZONE 'America/Los_Angeles' COMMENT 'This is my table comment' TBLPROPERTIES('test.key2' = 'test.value2') AS SELECT  tpep_pickup_datetime AS pickup_time,  AVG(trip_distance) AS avg_trip_distance FROM samples.nyctaxi.trips GROUP BY 1 ORDER BY 1
`;
        const expected = `
...
ORDER BY
  1
`;
        expect(sqlFormatter.format(streamingTableQuery.trim())).toBe(
            expected.trim(),
        );
    });

    it("handles streaming table", () => {
        const streamingTableQuery = `
CREATE STREAMING TABLE test.schema.materializedview SCHEDULE CRON '0 0 10 ? * *' AT TIME ZONE 'America/Los_Angeles' COMMENT 'This is my table comment' TBLPROPERTIES('test.key2' = 'test.value2') AS SELECT  tpep_pickup_datetime AS pickup_time,  AVG(trip_distance) AS avg_trip_distance FROM samples.nyctaxi.trips GROUP BY 1 ORDER BY 1
`;
...
`;
    ...
    it("supports fat arrows", () => {
        const query = `SELECT sql_func(5, paramA => 6);`;
        const expected = `
SELECT
  sql_func(5, paramA => 6);`
